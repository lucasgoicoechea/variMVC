		...
		if ($params !== array ()) // dynamic query
{
	...
	public function save($runValidation = true, $attributes = null) {
		if (! $runValidation || $this->validate ( $attributes )) {
			$this->usuario_log = UsersLogin::getAdministradorUserNameByUsersLoginID ( Yii::app ()->user->id );
			$this->fecha_log = CTimestamp::formatDate ( 'Y-m-d H:i:s' );
			if ($this->hasAttribute ( 'caja_id' )) {
				if ($this->getIsNewRecord ()) {
					$caja = Caja::getCajaAbierta ( CTimestamp::formatDate ( 'Y-m-d ' ) );
					$this->caja_id = $caja->id_caja;
					if ($caja->cerrada) {
						//mando cartel de error y return
						Yii::app()->user->setFlash('error', "<B>LA CAJA ESTA CERRADA</B>");
						return false;
					}
				} else {
					$caja = Caja::getById ( $this->caja_id );
					if ($caja->cerrada && !$this->getExeptuaCierre()) {
						//mando cartel de error y return
						Yii::app()->user->setFlash('error', "<B>LA CAJA ESTA CERRADA</B>");
						return false;
					}
