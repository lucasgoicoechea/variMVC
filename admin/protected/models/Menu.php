...
class Menu extends CActiveRecord {
	...
	public function getTreeLabel() {
		$html = '<span class="' . $this->class . '" style="font-size: 12px" ';
		if ($this->url == null)
			//$html = $html ." onclick=\"$(this).next('ul').slideToggle('slow');\" ";
			$html = $html ." onclick=\"$(this).nextAll().each(function() {
				if (this.tagName == 'ul') {
					$(this).slideToggle('slow');
			}});\" ";
		$html = $html .	'>';
		if ($this->img != null) {
			$htmlOptionImage = array (
					'width' => 16,
					'align' => 'top',
					'height' => 16 
			);
			$src = Yii::app ()->theme->baseUrl . '/img/' . $this->img;
			$html .= CHtml::imageButton ( $src, $htmlOptionImage );
		}
		$html .= $this->label;
		$html .= '</span>';
		return $html;
	 ...
	 *
	 * @param
	 *        	integer the id of the parent model
	 ...
	 * @param
	 *        	integer maximum number of names to be returned
	...
	public function getIDByURL($path){ //$path ejemplo pago/create
