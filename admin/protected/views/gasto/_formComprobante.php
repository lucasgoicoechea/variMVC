...
<?php echo CHtml::error($model,'FechaAsiento'); ?>
	  ...
	  $this->widget('application.extensions.moneymask.MMask',array(
                    'element'=>'#Gasto_Monto,#GastoItem_valor_unidad,#GastoItem_valor_final,#GastoItem_cantidad',
                    'currency'=>'PHP',
                    'config'=>array(
                        'symbolStay'=>true,
                        'thousands'=>'.',
...
jQuery("#telefono").val(ui.item["telefono"]);listComprobantes(); }',
		...
		<div class="contenedor-columna-30">
		 ...
		 <?php
			if (! isset ( $subcontrato )) {
				...
				if ($model->id_proveedor != '' && $model->id_proveedor != 0) {
					echo CHtml::textField ( "telefono", $model->proveedor->Telefono, array (
							'readonly' => true 
					) );
				} else {
	...
	if (isset ( $subcontrato )) {
		...
		if (isset($model->id_proveedor) && $model->id_proveedor>0 && isset($model->id_contrato_cabecera) && $model->id_contrato_cabecera>0) {
	...
	<?php if (isset($modelPago)){ ?>
		...
		<div class="contenedor-columna-60">
			...
			<?php
		if ($modelPago->id_cuenta != null && $modelPago->id_cuenta != '' && $modelPago->id_cuenta != 0) {
