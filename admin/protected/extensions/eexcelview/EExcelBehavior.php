...
class EExcelBehavior extends CBehavior {
	 ...
	 *
	 * @param
	 *        	null string|CDataProvider|array $model the data provider for the grid.
	 * @param
	 *        	null array $columns array with specif columns and its format. Defaults to all attributes.
	 * @param
	 *        	null string $title title for the file
	 * @param
	 *        	null array $documentDetails details of the document
	 * @param
	 *        	null string $exportType format to export (Excel5,Excel2007, PDF, HTML, CSV). Defaults to Excel2007 (xlsx)
	 */
	public function toExcel($model = null, $columns = array(), $title = null, $documentDetails = array(), $exportType = 'Excel2007') {
		...
		/**
		 * We need a data provider in order to create a CGridView.
		 * Actually we could pass a CActiveDataProvider instance or
		...
		$config = array (
				...
				'columns' => $columns,
				// agreagados por mi LGoicoechea
				'template' => "{summary}\n{items}\n{exportbuttons}\n{pager}" 
		);
		$arg = CMap::mergeArray ( $config, $documentDetails );
		$this->owner->widget ( 'ext.eexcelview.EExcelView', $arg );
		/*
		 * $this->owner->widget ( 'ext.eexcelview.EExcelView', array ( //'id' => 'pms-grid', 'autoWidth' => true, 'title' => $title, 'exportType' => $exportType, 'dataProvider' => $dataProvider, 'columns' => $columns, //'grid_mode' => 'export', //'stream' => false ) );
		 */
