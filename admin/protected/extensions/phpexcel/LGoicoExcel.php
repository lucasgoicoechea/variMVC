...
class LGoicoExcel {
	...
	public function writeSheetTemplateWtihOutData($template, $headers, $autor, $title, $titleSheet, $fromDataRow, $titles) {
		...
		Yii::log ( 'LGOICOEXCEL2:' . Yii::app ()->basePath . DIRECTORY_SEPARATOR . 'extensions' . DIRECTORY_SEPARATOR . "phpexcel" . DIRECTORY_SEPARATOR . "dp-template" . DIRECTORY_SEPARATOR . $template, CLogger::LEVEL_WARNING, 'EXCEL' );
		try {
		...
		Yii::log ( 'LGOICOEXCEL2.1:' . Yii::app ()->basePath . DIRECTORY_SEPARATOR . 'extensions' . DIRECTORY_SEPARATOR . "phpexcel" . DIRECTORY_SEPARATOR . "dp-template" . DIRECTORY_SEPARATOR . $template, CLogger::LEVEL_WARNING, 'EXCEL' );
		...
		if ($nTitles > 0) // AGREGA TITULOS Y SUBTITULOS
{
	...
	public function writeSheetActiveData($title, $titleSheet, $fromDataRow, $titles) {
		$nTitles = count ( $titles );
		if ($nTitles > 0) // AGREGA TITULOS Y SUBTITULOS
{
			$rowTitle = 1;
			foreach ( $titles as $title ) {
				...
				$range = 'A' . $rowTitle . ':' . 'M' . $rowTitle;
				$this->phpExcelTmp->getActiveSheet ()->setCellValue ( 'A' . $rowTitle, $title, true );
				$this->phpExcelTmp->getActiveSheet ()->getRowDimension ( $rowTitle )->setRowHeight ( 25 );
				$this->phpExcelTmp->getActiveSheet ()->mergeCells ( $range );
				$this->phpExcelTmp->getActiveSheet ()->getStyle ( $range )->applyFromArray ( $style );
			}
		}
		...
		// $column = LGoicoExcel::renderHeaders ( $objPHPExcel, $headers, $fromDataRow, $column );
		// Rename worksheet
		$this->phpExcelTmp->getActiveSheet ()->setTitle ( $titleSheet );
		foreach ( range ( 'A', 'Z' ) as $columnID ) {
			$this->phpExcelTmp->getActiveSheet ()->getColumnDimension ( $columnID )->setAutoSize ( true );
		}
	}
	...
	public function sendMailXLS(){
	    $objWriter = PHPExcel_IOFactory::createWriter ( $this->phpExcelTmp, 'Excel2007' );
		$bk_file = $this->titleTmp . '.xlsx';
		$objWriter->save('./excels/'.$bk_file);
		$mail = $this->preparedPHPMailer('Balance Mensual - Gastos', 'lucasgoicoechea@gmail.com', 'graduados@presi.unlp.edu.ar', 'PRUEBA', 'Gra_duados2016');
					$mail->AddAttachment('./excels/'.$bk_file, $bk_file);
					$mail->AddAddress ( 'lucasgoicoechea@gmail.com' );
		$mail->SMTPDebug  = 1;
				if (! $mail->Send ()) {
				Yii::log ( "Mailer Error: " .$mail->ErrorInfo, CLogger::LEVEL_ERROR, 'ENVIO DE MAILS');
				echo $mail->ErrorInfo;
				}
				Yii::app ()->end ();
	}
		...
		if ($nTitles > 0) // AGREGA TITULOS Y SUBTITULOS
{
	...
	public function writeData($nroSheet,$data,$titles, $titleSheet, $headers,$fromDataRow){
		// Set active sheet index to the first sheet, so Excel opens this as the first sheet
		$this->setActiveSheet($nroSheet);
		...
		if ($nTitles > 0) // AGREGA TITULOS Y SUBTITULOS
		{
			...
			foreach ( $titles as $title ) {
				...
				$range = 'A' . $rowTitle . ':' . 'M' . $rowTitle;
				$this->phpExcelTmp->getActiveSheet ()->setCellValue ( 'A' . $rowTitle, $title, true );
				$this->phpExcelTmp->getActiveSheet ()->getRowDimension ( $rowTitle )->setRowHeight ( 'auto' );
				$this->phpExcelTmp->getActiveSheet ()->mergeCells ( $range );
				$this->phpExcelTmp->getActiveSheet ()->getStyle ( $range )->applyFromArray ( $style );
			}
		}
		...
		if ($n > 0) {
			...
			$rowData = 0;
			for($rowCount; $rowCount < ($fromDataRow + $n); ++ $rowCount) {
				LGoicoExcel::renderRow ( $this->phpExcelTmp, $data, $rowData, $headers, $rowCount );
				$rowData ++;
			}
		}
		...
		// agrego los headers
		//$column = 0;
		//$column = LGoicoExcel::renderHeaders ( $this->phpExcelTmp, $headers, $fromDataRow, $column );
		
	}
	
	public function preparedPHPMailer($asunto, $email, $mailOrigen, $cuerpo,$pass_smtp) {
		Yii::import ( 'application.extensions.phpmailer.JPhpMailer' );
		$mail = new JPhpMailer ();
		$mail->IsSMTP ();
		$mail->SMTPAuth = true;
		// $mail->SMTPSecure='ssl';
		$mail->Host = "smtp.presi.unlp.edu.ar"; // SMTP a utilizar. Por ej. smtp.elserver.com
		$mail->Username =  substr($mailOrigen, 0, strpos($mailOrigen, '@')); // Correo completo a utilizar
		$mail->Password = $pass_smtp; // Contrase�a
		$mail->Port = 25; // Puerto a utilizar
		$mail->From = $mailOrigen; //$mailOrigen." Esta direccion electronica esta protegida contra spam bots. Necesita activar JavaScript para visualizarla "; // Desde donde enviamos (Para mostrar)
		$mail->FromName = substr($mailOrigen, 0, strpos($mailOrigen, '@'));
