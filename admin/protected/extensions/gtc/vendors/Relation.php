...
class Relation extends CWidget {
	...
	public function init() {
		if (! is_object ( $this->model )) {
			if (! $this->_model = new $this->model ())
				throw new CException ( Yii::t ( 'yii', 'Relation widget is not able to instantiate the given Model' ) );
		} else {
		...
		if (! is_object ( $this->_relatedModel ))
			throw new CException ( Yii::t ( 'yii', 'Relation widget cannot find the given Relation(' . $this->relation . ')' ) );
	...
	public function getRelatedData() {
		/*
		 * At first we determine, if we want to display all parent Objects, or if the User supplied an list of Objects
		 */
	...
	/*
	 * Renders one dropDownList per selectable related Element. The users can add additional entries with the + and remove entries with the - Button .
	 */
	public function renderManyManyDropDownListSelection() {
		...
		foreach ( $this->_relatedModel->findAll () as $obj ) {
			...
			echo CHtml::dropDownList ( 'rel-' . $obj->id . "-" . $this->getListBoxName (), $isAssigned ? $obj->id : 0, CHtml::listData ( array_merge ( array (
					'0' => $this->allowEmpty 
			), $this->_relatedModel->findAll () ), $this->relatedPk, $this->fields ) );
...
';
	...
	protected function renderAddButton() {
		...
		if (! $this->useLinkButton) {
			...
			echo CHtml::LinkButton ( is_string ( $this->showAddButton ) ? $this->showAddButton : 'New', array (
					'submit' => $link 
			) );
		}
	}
