...
class XTreeBehavior extends CActiveRecordBehavior {
	 ...
	 *
	 * @param
	 *        	model the instance of ActiveRecord
	 * @param bool $showRoot
	 *        	wether the absolute root node should be returned
	 ...
	 * @param bool $showRoot
	 *        	wether the root node should be displayed
	 * @param bool $showRoot
	 *        	wether the current node should be displayed
	 ...
	 *
	 * @param int $id
	 *        	the id of the relative root node
	 * @param bool $showRoot
	 *        	wether the relative root node should be displayed
	...
	public function fillTree($id = null, $showRoot = true) {
		...
		$administrador_publico = Yii::app ()->getSession ()->get ( 'publico_administrador' );
		//Yii::log ( "TIPO_ADMIN:" . $administrador_publico, CLogger::LEVEL_WARNING, 'MENUES' );
		//si no es el root AND (es un menu publico OR (no es un usuario Consultador AND tiene Acceso a la opción))
		if ($showRoot === false && 
				($owner->publico ||
						 ($administrador_publico==0 && 
						 		AdministradoresAccesos::existeAccesoByUsersLoginID ( $owner->id_acceso, $administrador_id )))) {
	...
	protected function getTreeSubItems() {
		...
		//Yii::log ( "TIPO_ADMIN:" . $administrador_publico, CLogger::LEVEL_WARNING, 'MENUES' );
		// si no es el root AND (es un menu publico OR (no es un usuario Consultador AND tiene Acceso a la opción))
		if ($owner->publico || ($administrador_publico == 0 && AdministradoresAccesos::existeAccesoByAdminID ( $owner->id_acceso, $administrador_id ))) {
			$subItems = array ();
			if ($owner->children) {
				foreach ( $owner->children as $child )
	...
	protected function formatTreeItem($model) {
		//Yii::log ( "MENU:" . $model->{$this->treeLabelMethod} () . "-URL:" . $model->{$this->treeUrlMethod} (), CLogger::LEVEL_WARNING, 'MENUES' );
		...
		return array (
				'text' => CHtml::link ( $label, $url, array (
						'id' => $model->getAttribute ( $this->id ) 
				) ),
				'id' => $model->getAttribute ( $this->id ),
	...
	protected function getWidth() {
		return $this->with === array () ? 'childCount' : CMap::mergeArray ( array (
				'childCount' 
		), $this->with );
	}
}